...
\begin{figure}[H]
  ...
  \caption{Perceptron transforming \emph{inputs} $[x_0,\, x_1,\, \ldots,\, x_N]$ to output $y_k$.} 
...
for e from 1 to EPOCHS: 
  foreach (X^j, t_j) in T: 
    ...
    for i from 0 to N: 
      update w_{ij}
\end{lstlisting}
  \caption{Perceptron learning. Developed from equations~\ref{eq:perceptron-error} and \ref{eq:perceptron-learning} we get an \emph{weight update rule}~\ref{eq:perceptron-learning-rule} which is applied in loop for element in $T$.} 
