...
&\approx -(h^{*}_j - h_j)t_k
\end{align} 
The approximation step could be made because $\phi'(\eta_j)$ has same sign as $(\eta^{*}_j - \eta_j) $ \citep{hinton1988learning, o1996bio}. This works better if the difference of activations is smaller and therefore the activation for the reconstructed pattern $o$  is made similar to target pattern $t$: 
\begin{equation}
